...
This module contains API-level tests that exercise CRUD, ordering and
permission behavior for Sections, Blocks, Elements, Projects and portfolio
rendering. Each test class and test method include concise Google-style
docstrings so generated documentation (pdoc) describes intent clearly.
"""
...
class SectionCRUDTests(APITestCase):
    """CRUD and ordering tests for the Section endpoints.

    Verifies authentication, ownership enforcement, ordering semantics and
    validation rules for the section list/detail endpoints.
    """

    def setUp(self):
        """Create two users and a portfolio fixture used across tests."""
    ...
    def test_auth_required(self):
        """Unauthenticated requests to the section list return HTTP 401."""
        url = reverse("section-list-create", kwargs={"portfolio_id": self.portfolio.id})
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_owner_enforced(self):
        """Only the portfolio owner can list or create sections for it."""
    ...
    def test_crud_and_ordering(self):
        """Create, reorder and delete sections while verifying ordering is compacted."""
    ...
    def test_validations(self):
        """Validation errors are raised for blank names, invalid order and bad config types."""
        self.client.force_authenticate(user=self.user)
        list_url = reverse("section-list-create", kwargs={"portfolio_id": self.portfolio.id})
...
class SectionPermissionTests(APITestCase):
    """Permission checks for section endpoints.

    Ensures that non-owners cannot access owner-only endpoints.
    """

    def setUp(self):
        """Create owner and attacker accounts and a section fixture."""
        User = get_user_model()
    ...
    def test_non_owner_cannot_access_section_detail(self):
        """A different user cannot retrieve the section detail (404)."""
...
class BlockCRUDTests(APITestCase):
    """CRUD and ordering tests for Block endpoints within sections."""

    def setUp(self):
        """Create users, portfolio and a section used in block tests."""
        User = get_user_model()
    ...
    def test_auth_required(self):
        """Unauthenticated access to block list returns HTTP 401."""
        url = reverse(
            "block-list-create",
            kwargs={"portfolio_id": self.portfolio.id, "section_id": self.section.id},
        )
    ...
    def test_owner_enforced(self):
        """Non-owners cannot list or create blocks for another user's section."""
    ...
    def test_crud_and_ordering(self):
        """Create blocks, reorder them and ensure ordering remains compacted."""
    ...
    def test_validations(self):
        """Validate block input fields and PUT semantics."""
        self.client.force_authenticate(user=self.owner)
        list_url = reverse(
            "block-list-create",
            kwargs={"portfolio_id": self.portfolio.id, "section_id": self.section.id},
    ...
    def test_grid_type_rejects_existing_mixed_element_sources(self):
        """Ensure converting a block to GRID enforces uniform element data_source."""
        self.client.force_authenticate(user=self.owner)
...
class ElementCRUDTests(APITestCase):
    """CRUD, ordering and validation tests for Element endpoints."""

    def setUp(self):
        """Prepare users, a portfolio, a section and a block for element tests."""
        User = get_user_model()
    ...
    def _list_url(self):
        """Return the element list/create URL for the fixture block."""
        return reverse(
            "element-list-create",
            kwargs={
                "portfolio_id": self.portfolio.id,
    ...
    def _detail_url(self, element_id: int):
        """Return the element detail URL for the given element id."""
        return reverse(
            "element-detail",
            kwargs={
                "portfolio_id": self.portfolio.id,
    ...
    def test_auth_required(self):
        """Element list requires authentication (HTTP 401 when missing)."""
        res = self.client.get(self._list_url())
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_owner_enforced(self):
        """Non-owners cannot list or create elements for another user's block."""
        self.client.force_authenticate(user=self.other)
        res = self.client.get(self._list_url())
        self.assertEqual(res.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_crud_and_ordering(self):
        """Create elements, reorder and delete while preserving compact ordering."""
        self.client.force_authenticate(user=self.owner)
    ...
    def test_validations_and_mapping(self):
        """Validate element inputs and ensure cross-field mapping is enforced."""
        self.client.force_authenticate(user=self.owner)
    ...
    def test_grid_block_rejects_mixed_data_sources(self):
        """GRID block type only allows elements with a single uniform data_source."""
        self.client.force_authenticate(user=self.owner)
...
class PortfolioRenderTests(APITestCase):
    """Tests for portfolio rendering endpoints (preview and public slug)."""

    def setUp(self):
        """Create owner and a portfolio with a simple section used for rendering tests."""
        User = get_user_model()
    ...
    def test_auth_required(self):
        """Preview rendering requires authentication for unpublished portfolios."""
        url = reverse("portfolio-render", kwargs={"portfolio_id": self.portfolio.id})
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_owner_only(self):
        """Only the owner can preview an unpublished portfolio."""
        self.client.force_authenticate(user=self.attacker)
        url = reverse("portfolio-render", kwargs={"portfolio_id": self.portfolio.id})
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_render_preview_unpublished(self):
        """Owner can preview an unpublished portfolio and see sections in payload."""
        self.client.force_authenticate(user=self.owner)
        url = reverse("portfolio-render", kwargs={"portfolio_id": self.portfolio.id})
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_public_render_by_slug_no_auth_for_published(self):
        """Published portfolios are accessible by slug without authentication."""
    ...
    def test_public_render_by_slug_hidden_when_unpublished(self):
        """Unpublished portfolios are not exposed via public slug."""
...
class PortfolioResumeUploadTests(APITestCase):
    """Tests that exercise resume upload and rendering integration."""

    def setUp(self):
        """Create a user used for resume upload tests."""
        User = get_user_model()
    ...
    def test_portfolio_resume_upload_and_render_preview(self):
        """Uploading a resume and verifying it appears in the rendered payload."""
        self.client.force_authenticate(user=self.user)
...
class ProjectFileUploadTests(APITestCase):
    """Tests for project image/file upload via multipart requests."""

    def setUp(self):
        """Create a user and portfolio fixture used in project upload tests."""
        User = get_user_model()
    ...
    def test_project_image_upload_via_multipart(self):
        """Verify an image uploaded via multipart is accepted and stored."""
        self.client.force_authenticate(user=self.user)
